...
def draw_mesh(ax, vertices, faces, joints=None, vertex_scalar=None, face_colors=None,
              title="", elev=12, azim=108):
...
def save_all_joint_weights_figure(path, vertices, faces, joints, lbs_weights):
    ...
    ax = fig.add_subplot(111, projection="3d")
    draw_mesh(ax, vertices, faces, joints=joints,
...
def prepare_posedirs(posedirs, expected_pose_dim):
    ...
    if posedirs.shape[1] == expected_pose_dim:
        return posedirs.T
    raise RuntimeError(
        f"posedirs 形状不匹配: {tuple(posedirs.shape)}, expected={expected_pose_dim}"
    )
...
# ---------- 选做：姿态动画 ----------
def generate_animation_frames(model, betas, joint_idx, angle_range, num_frames, out_dir):
    """生成关节旋转动画的多个帧"""
    device = betas.device
    dtype = betas.dtype
    os.makedirs(os.path.join(out_dir, "animation_frames"), exist_ok=True)
    ...
    for frame in range(num_frames):
        angle = angle_range[0] + (angle_range[1] - angle_range[0]) * frame / (num_frames - 1)
        # 假设目标关节是 right_shoulder (索引17)
        start = (joint_idx - 1) * 3
        body_pose[0, start:start+3] = torch.tensor([0.0, 0.0, angle], dtype=dtype, device=device)

        data = compute_manual_lbs(model, betas, global_orient, body_pose)
        verts = to_numpy(data["verts"][0])
        J_trans = to_numpy(data["J_transformed"][0])

        fig = plt.figure(figsize=(5, 6))
        ax = fig.add_subplot(111, projection="3d")
        draw_mesh(ax, verts, faces, joints=J_trans, title=f"Frame {frame+1}/{num_frames} (angle={angle:.2f})")
        fig.tight_layout()
        fig.savefig(os.path.join(out_dir, f"animation_frames/frame_{frame:04d}.png"), dpi=120, bbox_inches="tight")
        plt.close(fig)
        print(f"  Frame {frame+1}/{num_frames} saved")

    print(f"动画帧已保存到 {out_dir}/animation_frames/")
...
def main(args):
    ...
    if joint_id < 0 or joint_id >= model.lbs_weights.shape[1]:
        raise ValueError(f"joint_id 越界: {joint_id}, 范围 [0, {model.lbs_weights.shape[1]-1}]")
    ...
    save_comparison_grid(os.path.join(out_dir, "comparison_grid.png"), grid_dict, faces)
    ...
    if args.animate:
        ...
        generate_animation_frames(
            model, betas,
            joint_idx=args.animate_joint,
            angle_range=(-0.8, 0.8),
            num_frames=args.animate_frames,
            out_dir=out_dir
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--animate", action="store_true",
                        help="启用选做：生成姿态动画")
    parser.add_argument("--animate-joint", type=int, default=17,
                        help="动画驱动的关节索引 (默认17: right_shoulder)")
